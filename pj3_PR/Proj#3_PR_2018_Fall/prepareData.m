function [imgDataTrain, labelsTrain, imgDataTest, labelsTest, img1000Train, labels1000Train, img1000Test, labels1000Test] = prepareData
...
%% random number
perm1_6 = randperm(60000, 1000);
perm1_1 = randperm(10000, 1000);
...
%-----------------------------------------------------------------
rawImgDataTrain1000=rawImgDataTrain(:,:,perm1_6);
img1000Train(:,:,1,:) = uint8(rawImgDataTrain1000(:,:,:));
...
labels1000Train = labelsTrain(perm1_6);
fclose(fid);

% Process the labels
labelsTrain = categorical(labelsTrain);
labels1000Train = categorical(labels1000Train);
...
%-------------------------------------------
rawImgDataTest1000=rawImgDataTest(:,:,perm1_1);
img1000Test(:,:,1,:) = uint8(rawImgDataTest1000(:,:,:));
...
labels1000Test = labelsTest(perm1_1);
fclose(fid);

% Process the labels
labelsTest = categorical(labelsTest);
labels1000Test = categorical(labels1000Test);
